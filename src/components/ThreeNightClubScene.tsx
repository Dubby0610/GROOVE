  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (clubGltf : any) => {
        ...
        // Load the dancer model after the club model is loaded
        loader.load(
          "/models/dancer-draco.glb", // Changed from dancer-draco.glb to dancer.glb
          (dancerGltf : any) => {
            // Add the dancer to the scene
            const dancer = dancerGltf.scene;
            dancer.position.set(0, 0, 0); // Position the dancer
            dancer.scale.set(2.5, 2.5, 2.5);
            
            // Setup materials and shadows for dancer
            ...
            // Start animation loop after both models are loaded
          ...
          (error : any) => {
            ...
            // If dancer fails to load, still start the animation loop with just the club
            const clock = new THREE.Clock();
            const animate = () => {
              requestAnimationFrame(animate);
              const delta = clock.getDelta();
        ...
        const staticLights = [
          ...
          createStaticLights(0x002298, 0, 8, 5, 5000),
          // createStaticLights(0x002298, 0, -3, 0, 5000),        // Bottom of the floor
        ];
